...
class DatabaseManager:
    ...
    def get_all_tests(self, user_id=None):
        ...
        if user_id:
            cursor.execute("SELECT test_id, title, is_active FROM tests WHERE created_by = ? ORDER BY created_at DESC", (user_id,))
        else:
            cursor.execute("SELECT test_id, title, is_active FROM tests ORDER BY created_at DESC")
    ...
    def get_candidates_for_test(self, test_id):
        ...
        cursor.execute("""
            ...
            FROM candidates c
            LEFT JOIN personal_codes pc ON c.candidate_id = pc.candidate_id
            WHERE c.test_id = ?
    ...
    def get_candidate_results(self, candidate_id):
        ...
        cursor_users.execute("""
            ...
            JOIN personal_codes pc ON r.code = pc.code
            WHERE pc.candidate_id = ?
    ...
    def get_test_candidates_statistics(self, test_id):
        ...
        candidates_stats = cursor_tests.fetchall()
        conn_tests.close()
        ...
        for candidate in candidates_stats:
            ...
            # Получаем лучший результат кандидата
            cursor_users.execute("""
                SELECT MAX(r.score * 100.0 / r.total_questions), COUNT(r.result_id)
                FROM results r
                JOIN personal_codes pc ON r.code = pc.code
                WHERE pc.candidate_id = ?
            """, (candidate_id,))
            ...
            enhanced_stats.append({
                ...
                'total_codes': candidate[4],
                'used_codes': candidate[5],
